...
class VCControlView(discord.ui.View):
    ...
    async def lock_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not vc:
            return await interaction.followup.send("❌ আপনি কোনো ভয়েস চ্যানেলে নেই!", ephemeral=True)
        
        await vc.set_permissions(interaction.guild.default_role, connect=False)
        await interaction.followup.send("🔒 আপনার ভয়েস চ্যানেলটি লক করা হয়েছে।", ephemeral=True)
    ...
    async def unlock_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("🔓 আপনার ভয়েস চ্যানেলটি আনলক করা হয়েছে।", ephemeral=True)
    ...
    async def hide_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("👻 আপনার ভয়েস চ্যানেলটি হাইড করা হয়েছে।", ephemeral=True)
    ...
    async def unhide_vc(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("👀 আপনার ভয়েস চ্যানেলটি দৃশ্যমান করা হয়েছে।", ephemeral=True)
...
class VCSetupView(discord.ui.View):
    ...
    async def create_vc_button(self, interaction: discord.Interaction):
        # Prevent interaction timeout error immediately
        await interaction.response.defer(ephemeral=True)
        ...
        if existing_vc:
            if member.voice:
                try:
                    ...
                    pass
            return await interaction.followup.send(f"⚠️ আপনার ইতোমধ্যে একটি ভয়েস চ্যানেল খোলা আছে: {existing_vc.mention}", ephemeral=True)
        ...
        try:
            ...
            await interaction.followup.send(f"✅ আপনার ভয়েস চ্যানেল সফলভাবে তৈরি হয়েছে: {new_vc.mention}", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"❌ চ্যানেল তৈরি করতে সমস্যা হয়েছে: `{e}`", ephemeral=True)
